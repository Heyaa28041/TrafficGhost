...
export class BehaviorEngine {
  ...
  private mergeBehavior(...configs: BehaviorConfig[]): BehaviorConfig {
    ...
    const result: BehaviorConfig = { errorRates: {} as NonNullable<BehaviorConfig["errorRates"]> };
    for (const config of [...configs].reverse()) {
      ...
      for (const [code, rate] of Object.entries(config.errorRates ?? {})) {
        result.errorRates![Number(code) as keyof NonNullable<BehaviorConfig["errorRates"]>] = rate;
      }
    }
    return result;
  }
