...
export class ControlApi {
    ...
    constructor(workspaceRoot) {
        ...
        this.proxy = new ProxyCapture((record) => {
        ...
        this.source = project["source"] ?? undefined;
    }
    ...
    registerRoutes() {
        ...
        app.post("/import/har", async (req, reply) => {
        ...
        app.post("/import/har-json", async (req, reply) => {
            const { harJson, source } = req.body;
            if (!harJson)
                return reply.status(400).send({ error: "harJson is required." });
        ...
        // ─── Mock Server ──────────────────────────────────────────────────────────
        app.post("/mock/start", async (req, reply) => {
        ...
        app.post("/proxy/stop", async (_req, reply) => {
            if (!this.proxy.isRunning())
                return reply.status(400).send({ error: "Proxy is not running." });
            await this.proxy.stop();
            return { ok: true };
        });
