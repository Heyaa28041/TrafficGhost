...
function isDynamicSegment(segment) {
    if (!segment || STATIC_KEYWORDS.has(segment.toLowerCase()))
        return false;
    ...
    if (HASH_LIKE_RE.test(segment) && !/[a-zA-Z]{4,}/.test(segment))
        return true;
    return false;
}
...
function makeEndpointKey(method, normalizedPath) {
    return `${method.toUpperCase()}:${normalizedPath}`;
}
function headersToExample(record) {
    return {
...
export function detectEndpoints(records) {
    ...
    for (const record of records) {
        ...
        const alreadyHave = group.examples.some((e) => JSON.stringify({ s: e.status, b: e.body }) === exampleJson);
        if (!alreadyHave) {
            group.examples.push(headersToExample(record));
        }
    }
    const defaultBehavior = { latencyMs: 0, errorRates: {} };
    const endpoints = [];
