...
export function detectPagination(records, endpoints) {
    // Build a map: normalized path key → queries seen
    const queriesByEndpoint = new Map();
    for (const rec of records) {
        if (Object.keys(rec.query).length === 0)
            continue;
        const key = `${rec.method}:${rec.path}`;
        if (!queriesByEndpoint.has(key))
            queriesByEndpoint.set(key, []);
...
function detectPaginationForEndpoint(endpoint, queriesByEndpoint) {
    ...
    if (queries.length < 2)
        return null; // need multiple requests to detect pagination
...
//# sourceMappingURL=PaginationDetector.js.map