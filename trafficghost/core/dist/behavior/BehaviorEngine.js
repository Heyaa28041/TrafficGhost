...
export class BehaviorEngine {
    ...
    setGlobalBehavior(config) {
        this.globalBehavior = config;
    }
    setChaosMode(enabled, config) {
        this.chaosMode = enabled;
        if (config)
            this.chaosBehavior = config;
    }
    isChaosMode() {
    ...
    async applyBehavior(mock) {
        // Merge: chaos > per-endpoint > global
        const effective = this.mergeBehavior(this.chaosMode ? this.chaosBehavior : {}, mock.behavior, this.globalBehavior);
        // Apply latency
    ...
    mergeBehavior(...configs) {
        // Later entries take precedence (higher priority = listed earlier in args)
        const result = { errorRates: {} };
        for (const config of [...configs].reverse()) {
            if (config.latencyMs !== undefined)
                result.latencyMs = config.latencyMs;
            if (config.latencyRandom !== undefined)
                result.latencyRandom = config.latencyRandom;
            for (const [code, rate] of Object.entries(config.errorRates ?? {})) {
                result.errorRates[code] = rate;
            }
        }
        return result;
    }
    resolveLatency(config) {
        if (!config.latencyMs)
            return 0;
    ...
    evaluateErrorInjection(errorRates) {
        if (!errorRates)
            return null;
        const rand = Math.random();
        let cumulative = 0;
        // Evaluate in order: 500, 429, 404, 502, 503
        const order = [500, 429, 404, 502, 503];
