...
class DashboardPanel {
    static createOrShow(extensionUri, client) {
        ...
        const panel = vscode.window.createWebviewPanel("trafficghost.dashboard", "👻 TrafficGhost Dashboard", col, {
            enableScripts: true,
        ...
        switch (msg.command) {
            case "setBehavior": {
                ...
                if (msg.latencyMs !== undefined)
                    config.latencyMs = msg.latencyMs;
                if (msg.latencyRandom !== undefined)
                    config.latencyRandom = msg.latencyRandom;
                if (msg.errorRates)
                    config.errorRates = msg.errorRates;
            ...
            case "syncExasol": {
                try {
                    ...
                    this.panel.webview.postMessage({ type: "syncResult", ok: false, error: e.message });
                }
                break;
            }
        }
