...
import { GraphQLEndpointDefinition, MockResponseVariant } from '../models/endpoint';

export class GraphQLAnalyzer {
  /**
   * Checks if a captured request is a GraphQL operation.
   */
  public static isGraphQLRequest(req: CapturedRequest): boolean {
   ...
   */
  public static extractOperationDetails(req: CapturedRequest): {
    ...
    if (typeof req.body === 'object' && req.body !== null) {
      ...
      if (typeof bodyObj.variables === 'object' && bodyObj.variables !== null) {
        variables = bodyObj.variables as Record<string, unknown>;
      }
   ...
   */
  public static analyzeGraphQL(requests: CapturedRequest[]): GraphQLEndpointDefinition[] {
    ...
    for (const req of requests) {
      if (!GraphQLAnalyzer.isGraphQLRequest(req)) continue;
      ...
      const resVariant: MockResponseVariant = {
    ...
    const endpoints: GraphQLEndpointDefinition[] = [];
