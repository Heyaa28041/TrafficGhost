...
export class HarParser {
   ...
   */
  public static parse(
    harJsonContent: string,
    redactHeaderNames?: string[]
  ): CapturedRequest[] {
    let raw: HarRoot;
    try {
      raw = JSON.parse(harJsonContent) as HarRoot;
    } catch (err) {
    ...
    for (const entry of raw.log.entries) {
      ...
      if (!entry.request || !entry.request.url || !entry.request.method) {
        logger.warn(`Skipping malformed HAR entry #${index}: missing request method or url.`);
        continue;
      }
      ...
      try {
        ...
        logger.warn(`Error normalizing HAR entry #${index}: ${err instanceof Error ? err.message : String(err)}`);
      }
    }

    logger.info(`Successfully parsed ${captured.length} requests from HAR.`);
    return captured;
  }
  ...
  public static normalizeEntry(
    ...
    const rawReqHeaders: Record<string, string> = {};
    ...
    const headers = redactHeaders(rawReqHeaders, redactHeaderNames);
        ...
        if (q && q.name) {
          if (query[q.name] !== undefined) {
            ...
            if (Array.isArray(existing)) {
              ...
              query[q.name] = [existing as string, q.value || ''];
            }
          } else {
    ...
    if (req.postData?.text) {
      ...
      if (requestContentType && requestContentType.includes('application/json')) {
        ...
        for (const p of req.postData.params) {
          if (p.name) formObj[p.name] = p.value || '';
        }
        requestBody = formObj;
      } else {
        requestBody = rawText;
    ...
    const responseHeaders = redactHeaders(rawResHeaders, redactHeaderNames);
