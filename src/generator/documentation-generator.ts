...
export class DocumentationGenerator {
  public static generateMarkdown(schema: TrafficGhostMockSchema): GeneratedDocs {
    const lines: string[] = [];
  ...
  private static formatRestEndpoint(ep: RestEndpointDefinition): string {
    const lines: string[] = [];
  ...
  private static formatGraphQLEndpoint(g: GraphQLEndpointDefinition): string {
    const lines: string[] = [];
  ...
  private static formatBody(body: unknown, indent = 2): string {
    try {
